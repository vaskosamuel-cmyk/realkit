    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <button 
            ...
            className="group flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white text-sm font-semibold rounded-full hover:bg-slate-800 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-slate-900/30"
          >
            {content.cta}
      ...
      {mobileMenuOpen && (
        <div className="absolute top-full left-0 right-0 bg-white border-b border-slate-200 p-6 md:hidden shadow-xl flex flex-col gap-4 animate-in slide-in-from-top-5">
          <button onClick={() => scrollToSection('features')} className="text-left text-lg font-medium text-slate-900">{content.features}</button>
          <button onClick={() => scrollToSection('preview')} className="text-left text-lg font-medium text-slate-900">{content.preview}</button>
          <button onClick={() => scrollToSection('testimonials')} className="text-left text-lg font-medium text-slate-900">{content.reviews}</button>
          <button onClick={() => scrollToSection('contact')} className="text-left text-lg font-medium text-slate-900">{content.contact}</button>
          <button 
            onClick={() => scrollToSection('pricing')}
            className="w-full mt-2 flex items-center justify-center gap-2 px-6 py-3 bg-slate-900 text-white font-semibold rounded-xl shadow-lg shadow-slate-900/20"
          >
            {content.mobileCta}
          </button>
        </div>
