      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="lg:col-span-7 space-y-8 order-1 lg:order-1">
              <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 rounded-[2.5rem] p-8 md:p-12 relative">
                ...
                <ul className="space-y-4 mb-10">
                  {t.items.map((item, idx) => (
                    <li key={idx} className="flex justify-between items-center gap-4 py-4 border-b border-white/5 last:border-0 hover:bg-white/5 px-4 rounded-xl transition-colors">
                      ...
                      <span className="text-slate-500 font-semibold shrink-0 text-lg">{item.value}€</span>
                    </li>
                  ))}
                </ul>
                ...
                <div className="flex justify-between items-center pt-8 mt-4 border-t border-white/10 relative">
                  ...
                  <span className="text-4xl md:text-5xl font-bold text-slate-300">{totalValue}€</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 lg:sticky lg:top-28 order-2 lg:order-2">
              <div className="bg-white text-slate-900 rounded-[2.5rem] shadow-2xl shadow-black/50 overflow-hidden relative ring-4 ring-blue-500/20">
                
                <div className="p-8 md:p-12 text-center">
                  
                  <h3 className="text-xl md:text-2xl font-bold mb-6 text-slate-800">Váš balík obsahuje:</h3>

                  <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6 mb-8 relative text-left">
                    <div className="flex items-center gap-2 mb-4">
                      <Gift size={20} className="text-blue-600" />
                      <span className="text-blue-800 font-bold text-sm uppercase">Bonusy pre vás</span>
                    </div>
                    <div className="space-y-3">
                      {t.bonuses.map((bonus, i) => (
                        <div key={i} className="flex justify-between items-center gap-2 text-base">
                  ...
                  <div className="flex flex-col items-center justify-center gap-0 mb-4 relative">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-7xl md:text-9xl font-extrabold text-slate-900 tracking-tighter leading-none">
                        39€
                      </span>
                    </div>
                  </div>
                  ...
                  {/* Discount Badge */}
                  <div className="inline-block bg-yellow-400 text-slate-900 text-sm font-extrabold px-4 py-1.5 rounded-full uppercase tracking-wide mb-8">
                    Ušetríte Viac Ako 90%
                  </div>

                  {/* Urgency Timer */}
                  <div className="flex items-center justify-center gap-2 text-red-600 font-bold uppercase tracking-wide text-sm mb-8 animate-pulse">
                    <Clock size={18} />
                  ...
                  <button 
                    onClick={onOpenModal}
                    className="w-full py-6 bg-slate-900 text-white font-bold rounded-2xl shadow-xl hover:bg-slate-800 hover:scale-[1.02] active:scale-[0.98] transition-all text-xl flex items-center justify-center gap-3 mb-6"
                  >
                    {t.cta}
                    <ArrowRight size={24} />
                  </button>
                ...
                <div className="bg-slate-50 border-t border-slate-100 p-5 text-center">
                   <div className="flex justify-center -space-x-2 mb-3">
                      {[1,2,3,4,5].map(i => (
                        <img key={i} src={`https://picsum.photos/30/30?random=${i+20}`} className="w-8 h-8 rounded-full border-2 border-white" alt="Buyer" />
                      ))}
                   </div>
                   <p className="text-sm text-slate-500 font-medium">Zakúpilo už viac ako 2,400+ maklérov</p>
                </div>
              </div>
            </div>
